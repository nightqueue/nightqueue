...
// Runs `nightshift sandbox` as a real subprocess, with the given args after `sandbox` and env/cwd for the caller.
function runSandbox(t, args, { env, cwd } = {}) {
  ...
  const baseEnv = {
    ...process.env,
    NIGHTSHIFT_HOME: parentHome,
    CLAUDE_CONFIG_DIR: parentClaudeDir,
    NIGHTSHIFT_SANDBOX_PROBE_OTHER: "kept-across-the-spawn",
    ...env,
  };
...
test("the wrapped command sees a throwaway home and host config that exist during the run, differ from the caller's and inherit the rest", (t) => {
  ...
  assert.equal(report.homeExists, true, "NIGHTSHIFT_HOME must exist while the command runs");
  assert.equal(report.claudeConfigDirExists, true, "CLAUDE_CONFIG_DIR must exist while the command runs");
  assert.notEqual(report.home, result.parentHome, "the sandbox must not hand the caller's own NIGHTSHIFT_HOME to the command");
  ...
  assert.equal(existsSync(report.home), false, "the throwaway NIGHTSHIFT_HOME must be gone once the command returns");
...
test("the child's own exit code and stderr pass through unchanged, and the throwaway home is still removed on failure", (t) => {
  const result = runSandbox(t, [process.execPath, PROBE, "7"], { env: { NIGHTSHIFT_SANDBOX_PROBE_STDERR: "boom from the child" } });
  const report = probeReport(result.stdout);
...
test("a flag of the wrapped command reaches it instead of being consumed by nightshift", (t) => {
...
test("empty argv is a usage error", (t) => {
  ...
  assert.match(result.stderr, /nightshift sandbox <command> \[args\.\.\.\]/);
});

test("a command that cannot be spawned exits 127 with a message on stderr", (t) => {
  const result = runSandbox(t, ["nightshift-sandbox-command-that-does-not-exist"]);

  assert.equal(result.code, 127);
  assert.match(result.stderr, /nightshift-sandbox-command-that-does-not-exist/);
});
