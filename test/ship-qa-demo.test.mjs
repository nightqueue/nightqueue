...
import { DEMO_REMOTE, demoOriginRefusal, jobIdentityRefusal, REFUSAL_EXIT } from "../scripts/ship-qa-demo.mjs";
import { makeDir } from "../test-support/memory.mjs";
import { git } from "../test-support/worktrees.mjs";

const SCRIPT = fileURLToPath(new URL("../scripts/ship-qa-demo.mjs", import.meta.url));
...
test("the origin guard reads the real origin of a local checkout, and refuses the nightshift repository", (t) => {
  assert.match(demoOriginRefusal(repoWithOrigin(t, "ship-qa-nightshift", "git@github.com:maykonVinicius/nightshift.git")), /not maykonVinicius\/nstest-demo/);
  assert.equal(demoOriginRefusal(repoWithOrigin(t, "ship-qa-demo", `https://github.com/${DEMO_REMOTE}.git`)), null);
  assert.match(demoOriginRefusal(makeDir(t, "ship-qa-no-git")), /its origin is unreadable/);
});
...
test("run inside a job, the script refuses before anything else, even aimed at the nightshift repository, and exits non-zero", (t) => {
  const repo = repoWithOrigin(t, "ship-qa-spawn", "git@github.com:maykonVinicius/nightshift.git");
