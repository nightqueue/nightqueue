...
import { fakeShipDeps, openPr } from "../../test-support/ship.mjs";
...
// was still going, and #71 got merged onto job 57 by `queue ship` before this fix.
...
test("job 57, scenario A: the runtime QA's own scratch pull request on another branch never replaces the run's own recorded delivery", () => {
  ...
  const state = { schemaVersion: 1, slug: "queue-ship", type: "feature/refactor", branch: RUN_BRANCH, phases: [], outcome: { status: "done", prUrl: OWN_PR } };
...
test("job 57, scenario B: shipping the job's own recorded pull request never merges the QA's foreign pull request onto it", async (t) => {
  const env = makeHome(t, "job57-regression");
  const checkout = makeProject(t, env, "nightshift");
  const id = addJob({ project: "nightshift", prompt: "fix queue ship follow-ups" }, env).id;
  ...
  acquireShip(id, { worker: "ship:test:57:aaaa", leaseS: 660 }, env);
  const store = openStore(env);

  // PR #71 for real: its head is the QA's scratch branch, not job 57's branch.
  const fake = fakeShipDeps({ pr: openPr({ headRefName: QA_BRANCH }) });

  const outcome = await runShip({ store, job: getJob(id, env), worker: "ship:test:57:aaaa", env, deps: fake.deps, timeoutS: 600, checkout, force: false });
