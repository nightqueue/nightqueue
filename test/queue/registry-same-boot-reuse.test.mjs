...
import { stopRunner, writeRunnerRecord } from "../../src/queue/registry.mjs";
...
test("stop never signals a pid whose identity as the runner was not confirmed, even inside the same boot", { skip: "known limitation: a pid recycled inside the same boot session is still trusted; confirming real process identity needs ps//proc - tracked in a dedicated ticket" }, async (t) => {
  const env = makeHome(t, "registry-same-boot-reuse");
  const startedAt = new Date().toISOString();
  writeRunnerRecord({ pid: RECYCLED_PID, startedAt, mode: "watch", intervalS: 30, logPath: "/tmp/runner.log" }, env);

  const signals = [];
  await stopRunner({ pid: RECYCLED_PID, env, killImpl: fakeRecycledKill(RECYCLED_PID, signals), sleepImpl: async () => {} });
