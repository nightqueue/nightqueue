...
import { conflictStep, mergeStep, preflightStep, runShip, settleStep } from "../../src/queue/ship.mjs";
import { shipStoppedLine } from "../../src/queue/ship-view.mjs";
import { jobDetailView } from "../../src/queue/view.mjs";
import { openStore, withReadOnlyStore } from "../../src/store/open.mjs";
import { makeHome, makeProject } from "../../test-support/memory.mjs";
import { fakeShipDeps, gitFail, gitLines, gitOk, HEAD_SHA, MERGE_SHA, mergedPr, openPr, PUSHED_SHA, SHIP_PR_URL } from "../../test-support/ship.mjs";
...
function assertCanonicalUntouched(log, checkout) {
  for (const line of gitLines(log, checkout)) assert.match(line, CANONICAL_COMMANDS, `the ship ran \`git ${line}\` in the canonical checkout`);
  assert.equal(gitLines(log).some((line) => line.startsWith("stash")), false, "the ship stashed");
}

test("preflight stops at pr-closed on a closed pull request, the job status untouched and the resume line printed", async (t) => {
  const home = shipHome(t, "ship-steps-closed");
  const fake = fakeShipDeps({ pr: openPr({ state: "CLOSED" }) });
  const { outcome, row, checklist } = await ship(home, fake);
  assert.deepEqual(outcome, { status: "failed", step: "preflight", reason: "pr-closed", mergeSha: null, worktree: null });
  assert.equal(row.status, "done");
  assert.equal(row.ship_status, "failed");
  assert.deepEqual(checklist.failed, { step: "preflight", reason: "pr-closed" });
  assert.equal(shipStoppedLine(row), `⛔ ship stopped at preflight: pr-closed - run again with: nightshift queue ship ${home.id}`);
  assert.equal(fake.log.merges.length, 0);
});

test("a pull request already merged is recorded at preflight, never merged again, and settles the job with the Shipped line appended", async (t) => {
  const home = shipHome(t, "ship-steps-merged");
  const fake = fakeShipDeps({ pr: mergedPr() });
  const { outcome, row, checklist } = await ship(home, fake);
...
test("preflight stops at checks-red and checks-pending naming the checks, and never waits for them", async () => {
  const red = fakeShipDeps({ checks: { ok: true, checks: [], failing: ["lint", "e2e"], pending: ["docs"] } });
  ...
  const pending = fakeShipDeps({ checks: { ok: true, checks: [], failing: [], pending: ["e2e", "ci/legacy"] } });
  ...
  const unreadable = await preflightStep({ ctx: ctxFor(), deps: fakeShipDeps({ checks: { ok: false, checks: [], failing: [], pending: [] } }).deps });
...
test("preflight stops at checkout-dirty when a local change is in a file the pull would bring, and never stashes it", async () => {
  const fake = fakeShipDeps({ git: { "status --porcelain -z": gitOk(" M src/a.mjs\0?? notes.txt\0") }, diffNames: { ok: true, files: ["src/a.mjs"] } });
  ...
  const unreadable = fakeShipDeps({ git: { "status --porcelain -z": gitOk(" M src/a.mjs\0") }, diffNames: { ok: false, files: [] } });
...
test("preflight passes a dirty checkout whose changes the pull does not touch, and a clean one", async () => {
  ...
  const clean = await preflightStep({ ctx: ctxFor(), deps: fakeShipDeps().deps });
...
test("a failed git fetch origin is a warning carried on every later note of the checklist, never a failure", async (t) => {
  const home = shipHome(t, "ship-steps-fetch-warning");
  const fake = fakeShipDeps({ git: { "fetch origin": gitFail("fatal: Could not resolve host: github.com") } });
  const { outcome, checklist } = await ship(home, fake);
  assert.equal(outcome.status, "shipped");
...
test("conflict reads an UNKNOWN mergeability once more after a pause, and stops at mergeability-unknown when it stays so", async () => {
  ...
  const stuck = fakeShipDeps({ pr: unknown });
  ...
  const settled = fakeShipDeps({ reads: [unknown], pr: openPr() });
...
test("a conflicting pull request is rebased in a throwaway worktree, tested and force-pushed with a lease when green", async () => {
  const fake = fakeShipDeps({ pr: openPr(CONFLICTING) });
...
test("conflict markers a clean rebase left behind fail real-conflict before the suite runs", async () => {
  const fake = fakeShipDeps({ pr: openPr(CONFLICTING), git: { "diff --check": gitFail("", "src/a.mjs:3: leftover conflict marker\nsrc/a.mjs:9: leftover conflict marker\n") } });
  const result = await conflictStep({ ctx: ctxFor(), deps: fake.deps });
  assert.equal(result.reason, "real-conflict");
...
test("a red suite, a missing test script and a refused push stop the conflict step, and the worktree is always removed", async () => {
  ...
  for (const [changes, reason, note] of cases) {
    const fake = fakeShipDeps({ pr: openPr(CONFLICTING), ...changes });
    const result = await conflictStep({ ctx: ctxFor(), deps: fake.deps });
...
test("merge squashes at the verified head, re-reads the merge commit from GitHub and pulls the base best-effort", async () => {
  ...
  assert.equal(result.status, "done");
  assert.deepEqual(result.data, { merged: true, mergeSha: MERGE_SHA, mergedAt: "2026-09-21T10:00:00Z" });
  assert.deepEqual(fake.log.merges, [{ url: SHIP_PR_URL, matchHeadCommit: HEAD_SHA }]);
  ...
  const elsewhere = fakeShipDeps({ git: { "rev-parse --abbrev-ref HEAD": gitOk("feat/x\n"), "pull --ff-only": gitFail("never") } });
  ...
  const failedPull = fakeShipDeps({ git: { "pull --ff-only": gitFail("fatal: Not possible to fast-forward") } });
...
test("merge never takes gh's exit code as the evidence, in either direction", async () => {
  const onlyAutoMerge = fakeShipDeps({ merge: () => ({ ok: true, stderr: "" }) });
  ...
  const mergedAnyway = fakeShipDeps({
    merge: (world) => {
      world.pr = mergedPr();
...
test("merge refuses a head that moved and a pull request that conflicts again, reopening the steps to check", async () => {
  const moved = await mergeStep({ ctx: ctxFor({ headSha: HEAD_SHA }), deps: fakeShipDeps({ pr: openPr({ headRefOid: "9999999" }) }).deps });
  assert.equal(moved.reason, "head-moved");
  assert.deepEqual(moved.reopen, ["preflight", "conflict"]);
  const conflicted = fakeShipDeps({ pr: openPr(CONFLICTING) });
...
test("a re-run after a failure at merge never merges again: exactly one merge call across attempts", async (t) => {
  const home = shipHome(t, "ship-steps-merge-once");
  const fake = fakeShipDeps({ merge: () => ({ ok: true, stderr: "" }) });
  const first = await ship(home, fake);
  ...
  const second = await ship(reacquire(home, "ship:test:2:bbbb"), fake);
  ...
  const other = shipHome(t, "ship-steps-merge-once-recorded");
  const recorded = fakeShipDeps({
    merge: (world) => {
      world.pr = mergedPr({ mergeSha: null });
      return { ok: false, stderr: "exit 1" };
    },
  });
  assert.equal((await ship(other, recorded)).outcome.reason, "merge-without-sha");
  recorded.world.pr = mergedPr();
  assert.equal((await ship(reacquire(other, "ship:test:3:cccc"), recorded)).outcome.status, "shipped");
  assert.equal(recorded.log.merges.length, 1, "a merge recorded by data.merged ran again");
});

test("settle refuses a ship whose merge is not recorded with its commit", async () => {
  ...
  assert.equal(ready.data.noticeLine, "Shipped: PR #7 merged as abc1234 on 2026-09-21");
});

test("settle closes the job, releases its worktree, appends the Shipped line to the notice, and run_notice stays hidden", async (t) => {
  const { checkout } = publishedCheckout(t, "ship-steps-settle");
  const env = { ...makeHome(t, "ship-steps-settle"), ...gitVars() };
  saveConfig(addProject(loadConfig(env, { warn: () => {} }), { path: checkout, name: "alpha" }).config, env);
  const worktree = addWorktree(checkout, "feat+shipped");
  const id = addJob({ project: "alpha", prompt: "fix the worker" }, env).id;
  mkdirSync(logsDir(env), { recursive: true });
  writeFileSync(jobLogPath(id, env), doneStream({ notice: "the run's notice" }));
  openDb(env)
    .prepare("UPDATE jobs SET status = 'done', slug = 'shipped', pr_url = ?, notice_md = ?, result = ? WHERE id = ?")
    .run(SHIP_PR_URL, "the run's notice", JSON.stringify({ logPath: jobLogPath(id, env) }), id);
  recordRunFields({ project: "alpha", slug: "shipped", fields: { worktree: worktree.path }, env });
  acquireShip(id, { worker: "ship:test:1:aaaa", leaseS: 660 }, env);
  const home = { env, checkout, id, worker: "ship:test:1:aaaa", store: openStore(env) };

  const { outcome, row, checklist } = await ship(home, fakeShipDeps({ pr: mergedPr() }));
  assert.equal(outcome.status, "shipped");
  ...
  assert.equal("run_notice" in detail, false, "the Shipped line made the row's notice look replaced");
});
