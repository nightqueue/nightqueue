...
test("queue.closeTimeoutS accepts an integer between 60 and 3600, otherwise falls back to 600", () => {
  ...
  for (const invalid of [0, 59, 3601, -600, 90.5, "600", null, {}, undefined]) {
    assert.equal(normalizeConfig({ queue: { closeTimeoutS: invalid } }).queue.closeTimeoutS, 600, `\`${String(invalid)}\` was accepted`);
  }
