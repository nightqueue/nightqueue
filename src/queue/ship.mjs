...
export const SHIP_LEASE_SLACK_S = 60;
...
// The timeout and signal of a child call: its own timeout, never past the ship's deadline.
...
// Runs a cleanup git call with its own short timeout and no signal, so it runs even after the ship was aborted; never throws.
...
// Reads the pull request of the ship as GitHub has it now.
...
// The data a ship keeps about its pull request from one read.
function prData(pr) {
  return { prNumber: pr.number, title: pr.title, headBranch: pr.headRefName, baseBranch: pr.baseRefName, headSha: pr.headRefOid };
}

// The data that records a merged pull request, as GitHub reports it.
function mergedData(pr) {
  return { merged: true, mergeSha: pr.mergeSha ?? null, mergedAt: pr.mergedAt ?? null };
}
...
// Tells whether the checks of the pull request stop the ship, and what they said.
...
async function conflictStep({ ctx, deps }) {
  ...
  if (pr.state === "MERGED") return { status: "skipped", note: "the pull request is already merged", data: mergedData(pr) };
...
async function addThrowaway(ctx, deps, head) {
  let dir = null;
  try {
    dir = deps.fs.makeTempDir(`nightshift-ship-${ctx.jobId}-`);
  } catch (err) {
...
// Removes a temporary directory; one left behind never fails the ship.
...
// Rebases in the throwaway worktree, runs the suite and pushes only when it is green.
async function rebaseTestAndPush(ctx, deps, work) {
  ...
  const suite = await runSuite(ctx, deps, work.dir);
  ...
  return { status: "done", note: `rebased onto origin/${work.base}, suite green, pushed ${sha7(work.headShaBefore)} -> ${sha7(data.headSha)}`, data };
}
...
async function runSuite(ctx, deps, dir) {
  ...
  if (timeoutMs <= 0) return { problem: failed("timeout", "not enough of the ship's timeout is left to run the suite") };
...
// The done result of a merged pull request: its merge commit recorded and the canonical checkout pulled when it can be.
async function mergedResult(ctx, deps, { pr, note }) {
  const data = mergedData(pr);
...
function mergeWithoutSha({ pr, call }) {
  const data = pr?.ok && pr.state === "MERGED" ? { merged: true, mergedAt: pr.mergedAt ?? null } : {};
...
// Finishes a merge an earlier attempt recorded: re-read only for a missing merge commit, never merge again.
async function confirmRecordedMerge(ctx, deps) {
  if (ctx.data.mergeSha) return await mergedResult(ctx, deps, { pr: { mergeSha: ctx.data.mergeSha, mergedAt: ctx.data.mergedAt }, note: "already merged" });
  const pr = await readPr(ctx, deps);
  if (pr?.ok && pr.state === "MERGED" && pr.mergeSha) return await mergedResult(ctx, deps, { pr, note: "already merged" });
  return mergeWithoutSha({ pr, call: null });
}

// Squash-merges the pull request at the head the ship verified, and proves the merge by re-reading its merge commit.
async function mergeStep({ ctx, deps }) {
  if (ctx.data.merged) return await confirmRecordedMerge(ctx, deps);
  const pr = await readPr(ctx, deps);
  if (!pr?.ok) return unreadablePr(ctx, pr);
  if (pr.state === "MERGED") return await mergedResult(ctx, deps, { pr, note: "already merged" });
  if (pr.state === "CLOSED") return failed("pr-closed", `PR #${pr.number} was closed without being merged`);
  ...
  return await mergedResult(ctx, deps, { pr: reread, note: "squash-merged" });
}
...
async function settleStep({ ctx }) {
  ...
  const noticeLine = shippedLine({ number: ctx.prNumber, sha: ctx.data.mergeSha, at: ctx.data.mergedAt });
...
// The number of a pull request, from the ship's recorded data or its URL, or null.
...
// The checklist a ship attempt starts from: the stored one, with its steps and data guaranteed to be objects.
function startingChecklist(job) {
  const stored = parseShipChecklist(job?.ship) ?? {};
...
// Requires a positive, finite hard timeout, so a ship can never run without a deadline.
function requireTimeoutS(timeoutS) {
  if (Number.isFinite(timeoutS) && timeoutS > 0) return timeoutS;
  throw new UserError(`invalid ship timeout \`${String(timeoutS)}\`; expected a positive number of seconds`);
}
...
function abortedResult(reason) {
  const note = reason === "timeout" ? "the ship passed its hard timeout" : "the ship was interrupted";
...
// Stops the ship as failed at a step, releasing the lease; a lease that is not this worker's any more answers `lost`.
async function stopShip(run, { step, reason }) {
  run.checklist.failed = { step, reason };
  run.checklist.finishedAt = new Date(run.now()).toISOString();
  const released = await run.store.jobs.failShip(run.job.id, { worker: run.worker, ship: run.checklist });
...
function renewalSeconds(run) {
  return Math.ceil(Math.max(0, run.deadline.deadlineMs - run.now()) / 1000) + SHIP_LEASE_SLACK_S;
}
...
async function recordStep(run) {
  return await run.store.jobs.recordShipStep(run.job.id, { worker: run.worker, ship: run.checklist, leaseS: renewalSeconds(run) });
}

// Performs the terminal write of a settled ship: close the job, mark it shipped, then note where its worktree went.
async function settleShipped(run) {
  const { store, job, worker, checklist, deps } = run;
  checklist.finishedAt = new Date(run.now()).toISOString();
  const closeShipped = typeof deps.closeShipped === "function" ? deps.closeShipped : closeShippedJob;
  let closed = null;
  let refusal = null;
  try {
    closed = await closeShipped({ store, id: job.id, worker, ship: checklist, noticeLine: checklist.data.noticeLine, env: run.env });
  } catch (err) {
    refusal = err?.message ?? String(err);
  }
  if (!closed?.job) return await refuseSettle(run, refusal);
  if (closed.worktree) await store.jobs.noteShipWorktree(job.id, { worktree: closed.worktree });
  return outcomeOf("shipped", { step: "settle", checklist, worktree: closed.worktree ?? null });
}
...
async function refuseSettle(run, refusal) {
  ...
  const detail = refusal ?? `the job is \`${current?.status ?? "unknown"}\` and its ship lease is not this process's`;
  ...
  return await stopShip(run, { step: "settle", reason: "close-refused" });
}
...
async function advance(run, step, isLast) {
  ...
  if (result.status === "failed") return await stopShip(run, { step: step.name, reason: result.reason ?? "failed" });
  if (isLast && result.status === "done") return await settleShipped(run);
  if (isLast) return await stopShip(run, { step: step.name, reason: "not-settled" });
  return await recordOrStop(run, step.name);
}

// Writes the checklist after a non-terminal step; a lease taken over answers `lost`, a failed write stops the ship.
async function recordOrStop(run, name) {
  try {
    ...
    return await stopShip(run, { step: name, reason: "checklist-write-failed" });
  }
}
...
export async function runShip({ store, job, worker, env = process.env, deps = null, timeoutS, signal = null, now = Date.now, onStep = null, checkout, steps = SHIP_STEPS }) {
  ...
  try {
    ...
    return await stopShip(run, { step: "settle", reason: "not-settled" });
  } finally {
    deadline.disarm();
  }
}
