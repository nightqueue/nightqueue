...
function saveConfigAfterSecret({ config, ctx, name, org }) {
  try {
    ...
    throw new Error(
      `secret stored for \`${name}\`, but the config write failed: ${err?.message ?? String(err)}; run \`shift connection bind ${name} --org ${org}\``,
    );
  }
}
...
function saveSecretsAfterConfig({ secrets, ctx, name }) {
  try {
    ...
    throw new Error(
      `unbound \`${name}\` from all orgs, but the secret file write failed: ${err?.message ?? String(err)}; run \`shift connection remove ${name}\` again`,
    );
  }
}
...
async function runAdd(argv, ctx) {
  ...
  ctx.err(
    `shift: warning: org \`${org}\` already uses \`${result.occupiedBy}\` for ${values.type}; run \`shift connection bind ${name} --org ${org}\` to switch`,
  );
}
