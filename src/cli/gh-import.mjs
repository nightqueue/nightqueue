...
function manualHint() {
  return `store a token with \`echo "$GITHUB_TOKEN" | shift connection add ${NAME} --type ${TYPE}\``;
}
...
function storedBlocker(config, secrets, org) {
  ...
  if (hasConnection(secrets, NAME)) {
    return `connection \`${NAME}\` already exists; run \`shift connection bind ${NAME} --org ${org}\``;
  }
  return null;
}
...
function cliBlocker(status) {
  ...
  if (!status.authenticated) return `GitHub CLI is not authenticated; run \`gh auth login\` and then \`shift init --gh\``;
  return null;
}
...
async function wantsImport(ctx, { mode, login }) {
  ...
  if (!ctx.stdin?.isTTY) {
    ctx.out(`GitHub CLI is authenticated as ${login ?? "an unknown account"}; run \`shift init --gh\` to import its token as connection \`${NAME}\``);
    return false;
  }
