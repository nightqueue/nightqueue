...
const USAGE = "nightqueue reflect [--transcript <path>] [--session <id>]";
...
// Runs `nightqueue reflect`: extracts the lessons of a transcript now, in the foreground.
export async function run(argv, ctx) {
  ...
  if (!values.transcript) throw new UserError(`\`nightqueue reflect\` requires --transcript <path>; usage: ${USAGE}`);
